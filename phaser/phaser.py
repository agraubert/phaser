...
socket = Socket('docker.for.mac.localhost', 15679)
socket.settimeout(None)


def _report(output, content):
...
def main():
	...
	for variant in dict_variant_reads:
		...
		if ref_reads+alt_reads > 0:
			covered_count += 1;
			_report(stream_out,"\t".join([snp_dict['chr'],str(snp_dict['pos']),variant,snp_dict['alleles'][0],snp_dict['alleles'][1],str(ref_reads),str(alt_reads),str(ref_reads+alt_reads)+"\n"]))
	stream_out.close();
	...
	for block in final_haplotypes:
		...
		for bam_i in range(0,len(bam_list)):
			if bam_i not in haplo_count_bam_exclude:
				...
				if total_cov > 0:
					...
					_report(stream_out_ase,str_join("\t",fields_out)+"\n")
		...
		if args.output_network in variants:
			...
			_report(stream_out_network,"\t".join(["variantA","variantB","connections","inferred\n"]))
			nodes = [];
			#for item in hap_a_network + hap_b_network:
			hap_a_vars = [];
			for item in hap_a_network:
				if item[2] > 0:
					_report(stream_out_network,list_to_string(item,"\t")+"\n")
					nodes.append(item[0]);
					nodes.append(item[1]);
			...
			for item in set(nodes):
				...
				if alleles[0][var_index] == xallele:
					...
					assigned_hap = "B";
				_report(stream_out_network,item+"\t"+str(var_index)+"\t"+assigned_hap+"\n")
			stream_out_network.close()
			...
			for variant_b, allele_b in zip(variants, alleles[1]):
				if variant_a != variant_b:
					...
					if (dict_variant_reads[variant_a]['ref'] == allele_a and dict_variant_reads[variant_b]['ref'] == allele_b) or (dict_variant_reads[variant_a]['ref'] != allele_a and dict_variant_reads[variant_b]['ref'] != allele_b):
						...
						a_config = "cis";
					if a_config != "":
						_report(stream_out_allele_configs,"\t".join([variant_a,dict_variant_reads[variant_a]['rsid'],variant_b,dict_variant_reads[variant_b]['rsid'],a_config])+"\n")
	...
	if args.unphased_vars == 1:
				...
				for bam_i in range(0,len(bam_list)):
					if bam_i not in haplo_count_bam_exclude:
						...
						if total_cov > 0:
							...
							fields_out += ["",""];
							_report(stream_out_ase,"\t".join(fields_out)+"\n")
		...
		for variant in singletons:
			...
			_report(stream_out,dict_var['chr']+"\t"+str(dict_var['pos']-1)+"\t"+str(dict_var['pos'])+"\t"+str(1)+"\t"+str(1)+"\t"+out_name+"\t"+dict_var['alleles'][0]+"|"+dict_var['alleles'][1]+"\t"+str(len(dict_var['read_set'][0]))+"\t"+str(len(dict_var['read_set'][1]))+"\t"+str(total_cov)+"\t"+str(0)+"\t"+str(0)+"\t"+phase_string+"\t"+str(float('nan'))+"\t"+phase_string+"\t"+str(float('nan'))+"\n")
